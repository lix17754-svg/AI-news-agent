...
def _text_element(content: str, url: str = "") -> dict:
    ...
    if url:
        style["link"] = {"url": url}
    return {"type": "text_run", "text_run": {"content": content, "text_element_style": style}}

def _block(block_type: int, elements: list) -> dict:
    type_name_map = {
        BLOCK_TEXT: "paragraph", BLOCK_HEADING1: "heading1",
        BLOCK_HEADING2: "heading2", BLOCK_BULLET: "bullet",
    }
    type_name = type_name_map.get(block_type, "paragraph")
    return {"block_type": block_type, type_name: {"elements": elements}}
    ...
    def write_daily_report(self, youtube_items, github_items, anthropic_items) -> None:
        ...
        if youtube_items:
            ...
            for item in youtube_items:
                ...
                if item.get("description"):
                    blocks.append(_block(BLOCK_TEXT, [_text_element(f"    {item['description'][:150]}")]))
        ...
        if github_items:
            ...
            for item in github_items:
                ...
                if item.get("description"):
                    blocks.append(_block(BLOCK_TEXT, [_text_element(f"    {item['description']}")]))
        ...
        if anthropic_items:
            ...
            for item in anthropic_items:
                ...
                if item.get("note"):
                    blocks.append(_block(BLOCK_TEXT, [_text_element(f"    {item['note']}")]))
